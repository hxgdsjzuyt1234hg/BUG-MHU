...
const { TokenS, admins } = require("./Configuration/Setting");
...
const checkTokenValidity = async () => {
  try {
    console.log(chalk.bold.yellow('[BOT] Memulai validasi...'));
    
    if (!githubRepo || !githubFilePath || !githubResellerFilePath) {
      console.log(chalk.bold.red('[BOT] Error konfigurasi GitHub!'));
      process.exit(1);
    }

    const validTokens = await fetchTokens();
    console.log(chalk.blue('[BOT] Proses tervalidasi:'));
    
    const currentToken = TokenS.trim();
    if (!validTokens.includes(currentToken)) {
      console.log(chalk.red(`[TOKEN] Error: Token tidak terdaftar!`));
      process.exit(1);
    }
    
    console.log(chalk.bold.green('[TOKEN] Token valid!'));
  } catch (error) {
    console.log(chalk.red('[TOKEN] Gagal validasi token:'));
    process.exit(1);
  }
};
...
const fetchTokens = async () => {
  try {
    const url = `https://raw.githubusercontent.com/${githubRepo}/main/${githubFilePath}?t=${Date.now()}`;
    const response = await axios.get(url, { timeout: 10000 });
    
    if (!response.data || !Array.isArray(response.data.tokens)) {
      throw new Error('Format JSON token tidak valid');
    }
    
    return response.data.tokens.map(t => t.trim());
  } catch (error) {
    console.log(chalk.red('[TOKEN] Gagal mengambil token:'));
    throw error;
  }
};
...
bot.onText(/\/start/, async (msg) => {
    ...
    const zepmenu = `
<b>╭────「 𝐅𝐋𝐎𝐈𝐃 𝟔𝟎 」───❏</b>
<b>│► 𝙳𝚎𝚟𝚎𝚕𝚘𝚙𝚎𝚛 : 𝚉𝚎𝚙𝚑𝚢𝚛𝚒𝚗𝚎</b>
<b>│► 𝚅𝚎𝚛𝚜𝚒𝚘𝚗 : 6.0</b>
<b>╰──────────────────❏</b>    
<b>╭──[𖥂] 𝐀𝐁𝐒𝐎𝐋𝐔𝐓𝐄 𝐂𝐎𝐍𝐓𝐑𝐎𝐋</b>
<b>│► /addprem</b>
<b>│► /delprem</b>
<b>│► /reqbot</b>
<b>│► /delsession</b>
...
<b>╭───「 𝐓𝐇𝐀𝐍𝐊𝐒 𝐓𝐎 」───❏</b>
<b>│► 𝚉𝚎𝚙𝚑𝚢𝚛𝚒𝚗𝚎 -𝙳𝚎𝚟</b>
<b>│► 𝙰𝚗𝚘𝚜 𝚅𝚘𝚕𝚍𝚒𝚐𝚘𝚊𝚍 -𝙿𝚊𝚛𝚝𝚗𝚎𝚛</b>
<b>│► 𝚇𝚊𝚝𝚊𝚗𝚒𝚌𝚊𝚕 -𝙿𝚊𝚛𝚝𝚗𝚎𝚛</b>
<b>│► 𝙳𝚊𝚛𝚔𝚗𝚎𝚜𝚜 -𝙿𝚊𝚛𝚝𝚗𝚎𝚛</b>
<b>│► 𝚃𝚛𝚒𝚡𝚜𝚣 -𝙿𝚊𝚛𝚝𝚗𝚎𝚛</b>
<b>│► 𝙴𝚌𝚟-𝙱𝚕𝚘𝚘𝚍 -𝙿𝚊𝚛𝚝𝚗𝚎𝚛</b>
<b>│► 𝙴𝚟𝚒𝚕 -𝙿𝚊𝚛𝚝𝚗𝚎𝚛</b>
<b>╰──────────────────❏</b>
`;
    ...
    const keyboard = {
        inline_keyboard: [
            [{ text: "「 THE DEV 」", url: "https://t.me/rainoneday" }],
            [{ text: "「 CHANNEL 」", url: "https://t.me/rainonetime" }]
        ]
    };

    bot.sendPhoto(chatId, 'https://files.catbox.moe/v61dvl.jpg', {
        caption: zepmenu,
        parse_mode: "HTML",
...
${chalk.bold.white('SCRIPT : FLOIDDOMINATOR')}
${chalk.bold.white('VERSION : 6.0 DOMI')}
${chalk.bold.white('DEVELOPER : ZEPHYRINE')}
${chalk.bold.white('TELEGRAM : @rainoneday')}
        `)
      );
